...
> 最近更新：2026-09-23 03:50 Asia/Shanghai · 负责人：游戏grok
...
- **窄屏装箱沉浸**：去掉 `body.packing .crate-panel{display:none}`（暂存 `#stagingArea` 在其内）；装箱时暂存可见+可拖；评测/数据面板改为底栏 sheet，开箱起自动收起。
- **移动端沉浸**：`body.packing` 隐藏帮助噪点/部分顶栏（不再整块藏 crate-panel）；仓库格 `fitCellSizeToWrap` 按容器宽适配，避免 8×8 右侧被裁。
...
2. 移动/窄屏：`?eval=1`，开箱后仓库格完整、装箱时暂存卡+形状可见、评测面板为底栏/装箱时自动收起
...
> 最近更新：2026-09-23 03:31 Asia/Shanghai · 负责人：游戏测试玩家  
> 截图：`docs/playtest-shots/line1-retest2/`、`docs/playtest-shots/line-followball/`

### 已测线路
| 线路 | 状态 | 备注 |
|---|---|---|
| 开箱灰阻塞复验 | ✅ | 清档后普通柜可开、可装箱结算 |
| 蜜月 5 场 / ¥45k 重跑 | 🔄 进行中 | 约至 **3/5**：现金 ≈¥20.9k、累计盈亏 ≈+¥8.5k（`02-honeymoon-r3-pack.png`）；后可自然结束蜜月进入青铜厅 |
| 柜差（普通→精选→密封） | ⏳ 浅摸 | 蜜月内以普通为主；精选仅点过；密封在蜜月后因现金不足难持续 |
| 贵货鉴宝 / 跟随球 | ⏳ 部分 | 答题路径已碰到（大金锭鉴宝失败 UI，`quiz-if-any.png`）；跟随球需评测「强制跟随球 / 软鉴宝 / +¥5万」才稳触发 |
| 结算华丽对比 | ⏳ 未比 | 仍默认「流畅」 |
| 冲资金 ~30 万 | ⏳ 未到 | 蜜月后现金常掉到约 ¥4k |

### 评测 6 问进度
1–2（开局/装箱）有体感；3 柜差只摸到精选；4 贵货仅碰到答题失败、跟随球手感未立住；5 华丽未比；6 冲资金未到。

### 新 bug / 体验点
1. **「空间紧张」误报**：仓库占用约 **28%（7/25）** 仍弹「空间紧张，请优先高价货」（蜜月 r3 装箱屏）。
2. **蜜月后现金门槛卡住贵货线**：蜜月结束账本提示经济回正后，场次 6–9 现金常仅约 ¥4k，只买得起普通柜（租金 ¥3.9k–¥4.5k），密封/高价货与跟随球难自然出现；评测面板开「强制跟随球」「软鉴宝门槛」+ 加现金才能继续测（`bug-cash-gated.png`、`eval-panel.png`）。
3. 开箱/结算链路本轮复验 **无新阻塞**。
...
|---|---|
| 开箱灰阻塞 | 可维持 ✅ |
| 蜜月过短 | 偏 ✅ 倾向（已能打过多场；完整 5/¥45k 仍建议再清档收尾一眼） |
| 贵货守住 / 跟随球 | 仍 ⏳；优先修「蜜月后买不起密封 → 测不到跟随球」的评测可达性，或给评测档稳定刷贵货入口 |
| 「空间紧张」文案 | 建议按占用率或剩余格数阈值再触发，避免早期误报 |

### 试玩日志
- **2026-09-23 03:06** · line1-retest2：蜜月约 3/5，现金 ¥20,924，盈亏 +¥8,524；开箱装箱结算正常；占用 28% 仍「空间紧张」。
- **2026-09-23 03:07–03:27** · line-followball：蜜月中碰到高价鉴宝答题失败（大金锭）；蜜月后现金闸门明显；评测强制跟随球开着仍难靠自然经济验证手感；截图见同目录。
