...
> 最近更新：2026-09-23 03:56 Asia/Shanghai · 负责人：游戏测试玩家  
> 截图：`docs/playtest-shots/line1-retest2/`、`docs/playtest-shots/line-followball/`、`docs/playtest-shots/line3-narrow/`

### 已测线路
| 线路 | 状态 | 备注 |
|---|---|---|
| 开箱灰阻塞复验 | ✅ | 清档后普通柜可开、可装箱结算 |
| 蜜月 5 场 / ¥45k 重跑 | ✅ 偏过 | 已能自然结束蜜月进青铜/翡翠侧；本轮已到翡翠厅场次约 15–17 |
| 柜差（普通→精选→密封） | 🔄 进行中 | 评测加现金后可开密封（租金约 ¥32.4k）；豪华柜 ¥64.8k 仍常买不起 |
| 贵货鉴宝 / 跟随球 | 🔄 部分 | 强制跟随球下已打到高命中结算（准确率 100% · 195/195，`ball-result-high.png`）；自然经济触发仍依赖评测加钱/强制开关 |
| 窄屏装箱沉浸（line3） | ❌ 复验未过 | 对照 ⑩「窄屏装箱沉浸」交付：装箱时暂存区仍不可见、货品在装箱屏不显示；结算却报「遗弃暂存 7 件」（`bug-staging-hidden.png` / `bug-packing-items-hidden.png` / `retest-04-settle.png`） |
| 结算华丽对比 | ⏳ 未比 | 仍默认「流畅」 |
| 冲资金 ~30 万 | ⏳ 未到 | 翡翠厅现金约 ¥5万级（含评测 +¥5万）；未冲到赤金门槛 |

### 评测 6 问进度
1–2（开局/装箱）有体感，且 **窄屏装箱暂存/货品隐藏** 把「装箱初感」打成阻塞级；3 柜差已摸到密封；4 贵货：答题失败 + 跟随球高命中均已碰到，自然触发手感仍未立住；5 华丽未比；6 冲资金未到。

### 新 bug / 体验点
1. **窄屏装箱：暂存区隐藏**（仍复现）：`body.packing` 下暂存卡不可见；`retest-01/02` 与 `bug-staging-hidden` 同屏，修后复验未过。
2. **窄屏装箱：货品不显示**：装箱屏仓库格空（0/25），但放弃结算显示遗弃暂存 7 件 / ¥6,592——玩家看不见可拖货却被结算遗弃（`bug-packing-items-hidden.png` → `retest-04-settle.png`）。
3. **评测底栏 sheet 缺失 / 面板叠层**：评测面板仍大块叠在大厅/开箱区，未见稳定底栏 sheet（`bug-eval-panel-overlap.png`、`bug-eval-sheet-missing.png`）。
4. **「空间紧张」误报**（仍开）：占用约 28%（7/25）仍弹「空间紧张…」（上轮遗留）。
5. **蜜月后现金门槛**（仍开）：无评测加钱时密封/跟随球难自然出现；本轮靠「+¥5万 / 强制跟随球」才补到跟随球高命中样本。
6. 开箱灰 / 放弃装箱结算链路本轮 **无新阻塞**（亏损租金结算正确）。
...
| 开箱灰阻塞 | 维持 ✅ |
| 蜜月过短 | 可改 ✅（已自然结束并进翡翠厅） |
| 窄屏装箱沉浸 / PC·移动复验 | 建议改 ❌ 或 🔄 打回；⑩ 声称已修，试玩复验未过 |
| 贵货守住 / 跟随球 | 仍 ⏳→🔄；玩法可达，优先补「自然经济能摸到密封/跟随球」与跳过/低命中降级样本 |
| 「空间紧张」文案 | 仍建议按占用率/剩余格阈值再触发 |
| 评测 UI（底栏 sheet） | 建议 🔄；叠层影响报 bug / 调参 |
